...
const fileItem = tv({
  slots: {
    ...
    icon: 
      'rounded-full border-4 border-teal-100 bg-teal-200 text-teal-600',
    deleteButton: '',
  },
           ...
           <UploadCloud className="h-4 w-4"/>
            ...
            {state === 'error' ? (
              ...
              <div className="flex flex-col">
                <span className="text-sm font-medium font-zinc-700 text-error-700">Upload failed, please try again.</span>
                ...
                <div className="flex flex-1 flex-col items-start gap-1">
                <div className="flex flex-col">
                  <span className="text-sm font-medium font-zinc-700">{name}</span>
                  <span className="text-sm text-zinc-500">{formatBytes(size)}</span>
                </div>
  
                <div className="flex w-full items-center gap-3">
                  <div className="h-2 flex-1 rounded-full bg-zinc-100">
                    <div className="h-2 rounded-full bg-teal-600" style={{ width: state === 'complete' ? '100%' : '80%' }}/>
